...
// The dashboard body, keyed by org — rendered inside the portal and,
// blurred, behind the pitch gateway.
export default async function DashboardContent({ orgId }: { orgId: string }) {
  const [dbDeals, pendingApprovals, recentLeads, dbReviews] = await Promise.all([
    prisma.deal.findMany({ where: { orgId } }),
    prisma.approval.findMany({ where: { orgId, status: 'pending' }, orderBy: { createdAt: 'desc' }, take: 6 }),
    prisma.lead.findMany({ where: { orgId }, orderBy: { createdAt: 'desc' }, take: 6 }),
    prisma.review.findMany({ where: { orgId } }),
  ]);
